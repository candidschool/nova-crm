...
const Stage8ActionButton = ({ 
  ...
  parentsName, // ← Added parentsName for userName field
  phone 
}) => {
  const [isLoading, setIsLoading] = useState(false);
  const [showHover, setShowHover] = useState(false);

  // ← Function to validate required parameters with field_key support
  const validateParameters = () => {
    ...
    // Optional: Add parentsName validation if needed
  ...
  const handleClick = async () => {
    ...
    setIsLoading(true);
    try {
      // ← API call to send WhatsApp message - FIXED: Added userName field
      const response = await fetch('https://backend.aisensy.com/campaign/t1/api/v2', {
        ...
        body: JSON.stringify({
          ...
          campaignName: 'registered',
          destination: phone,
          userName: parentsName || 'User', // ← ADDED: userName field (required by API)
          templateParams: [] // ← No template parameters needed for this message
        })
      });
      ...
      console.log('Stage 8 API Response:', result);
  ...
  const hoverMessage = `We have received the Registration amount of Rs.1000/-. Thank You.`;
